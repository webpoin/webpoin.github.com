(function(){
    ...
    TouchSlide({
        ...
        mainCell: '.content-bd',
        endFun: function(i, j){
            var ancher = $('#nav li').eq(i),
                left = ancher.offset().left,
                ow = ancher.outerWidth(),
                winWidth = $(window).width();

            if((winWidth - left) < ow){
                globalNavScroll.scrollToElement(ancher[0], null, null, true);
            }else if((left - 10) < Math.abs(globalNavScroll.x)){
                var index = (i - 1);
                index = index >= 0 ? index : 0;

                globalNavScroll.scrollToElement($('#nav li').eq(index)[0], 500, -10, 0);
            }
        }
    });
