		...
		function initLoad(){
			var top = $(document.body).height()/2 + 60;
			 $('.anim').on('anim',function(){
			 	var anim = $(this).show();
			 	// 阴影
			 	var shadow_img = anim.find('.shadow').css({top:50,opacity:0}).animate({top:16},400).get(0);
				// 元宝
				var gold_img = anim.find('.gold').css('top', -top).animate({top:-18},{duration: 1200,easing:'easeInQuad',step:function(now, fx){
					var x = (now+top)/(top-18);
					if(x>0.5){
			...
			setTimeout(function(){audio.play();},1100);
			// 消失
			setTimeout(function(){$('.anim').fadeOut(1000)},5000);
		}